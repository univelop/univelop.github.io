...
| [`contains(text,sub)`](/docs/formulas/childs/text-formulas.html#textformeln-1) | Gibt Ja/Nein abhängig davon zurück, ob der Text *sub* im Text *text* vorhanden ist. | `contains("Hallo Welt","Welt")`  liefert true
| [`textLength(value)`](/docs/formulas/childs/text-formulas.html#textformeln-1)  | Gibt die Textlänge inklusive Leerzeichen aus | `textLength("Hallo Welt")` liefert 10
| [`firstLetters(value,count)`](/docs/formulas/childs/text-formulas.html#textformeln-1) | Zählt die **ersten** Zeichen des Bausteinwertes anhand der vorgebenen Länge (count = Zahl)| `firstLetters("Hallo Welt",5)` liefert "Hallo"
| [`lastLetters(value,count)`](/docs/formulas/childs/text-formulas.html#textformeln-1) | Zählt die **letzten** Zeichen des Bausteinwertes anhand der vorgegeben Länge (count = Zahl) | `lastLetters("Hallo Welt",4)` liefert "Welt"
| [`firstWords(value,count)`](/docs/formulas/childs/text-formulas.html#textformeln-1)  | Zählt die **ersten** Wörter des Bausteinwertes anhand der vorgegebenen Anzahl (count = Zahl) | `firstWords("Hallo du große Welt",2)` liefert "Hallo du"
| [`lastWords(value,count)`](/docs/formulas/childs/text-formulas.html#textformeln-1) | Zählt die **letzten** Wörter des Bausteinwertes anhand der vorgegebenen Anzahl (count = Zahl) | *Der Stopp der Zählung erfolgt nach einem Leerzeichen und nur nach einem Leerzeichen*
| [`replace(text,find,sub)`](/docs/formulas/childs/text-formulas.html#textformeln-1)  | Ersetzt jedes vorkommen des Textes *find* im Text *text* und ersetzt ihn mit *sub* | `replace("Hallo Welt", "Welt", "Mond")` liefert "Hallo Mond"
...
| [`formatNumber(number, dezimalCount, thousandSeparator)`](/docs/formulas/childs/text-formulas.html#textformeln-1) | Formatiert die Nummber zu gewünschtem Format | `formatNumber(1000,2,true)` das Ergebnis *1.000,00*  
